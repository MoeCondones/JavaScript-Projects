# JavaScript-Projects

<h1> Hey there! I'm Mohamed 👋 </h1>
<h2> A Passionate Software Developer From The UK/Sweden </h2>
<img align="right" alt="GIF" src="https://camo.githubusercontent." width="300"/>
<h3> 👨🏻‍💻 About Me </h3>

- 🔭 &nbsp; I’m Soon Graduating a Full-Stack .NET BootCamp 
- 🤔 &nbsp; Exploring new technologies and developing software solutions and quick hacks.
- 💼 &nbsp; full-stack developer.
- 🌱 &nbsp; Enthusiast in full-Stack web development .
- ✍️ &nbsp; Always trying to improve my programming skills.
- 😴 &nbsp; I belive, a perfect nap can be the ultimate solution for any stress. 

<h3>🛠 Tech Stack</h3>

- 💻 &nbsp; ASP.NET |ASP.NET CORE | Entity Framework | ASP.NET MVC | MSSQL | JQuery | JavaScript | React | C#   
- 🌐 &nbsp; Full-Stack | React | HTML | CSS | JavaScript | Bootstrap |    
- 🛢 &nbsp; MySQL | Firebase | Postman
- 🔧 &nbsp; Visual Studio | Visual Studio code  | Git
- 🖥 &nbsp; Photoshop 


